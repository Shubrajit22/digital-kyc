...
export default function AadhaarUploadPage() {
  ...
  const next = async () => {
    ...
    if (!data.ok) {
      setError(data.message || "Aadhaar validation failed. Please re-upload.");
      return;
    }
    ...
    <main className="min-h-screen bg-gradient-to-b from-[#EEF4FF] to-white pb-10">
      ...
      <div className="max-w-3xl mx-auto mt-6 bg-white rounded-2xl border border-slate-200 shadow-xl p-8 md:p-10 space-y-10">
        ...
        <div className="flex justify-between">
          ...
          >
            {loading ? (
              <span className="animate-spin h-5 w-5 cursor-pointerborder-2 border-white border-t-transparent rounded-full"></span>
            ) : (
              <>Continue →</>
            )}
          </button>
